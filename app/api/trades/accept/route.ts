...
export async function POST(req: Request) {
  try {
    ...
    if (trade.toUserId !== decoded.userId) {
      return NextResponse.json(
        { success: false, message: "Solo il destinatario può accettare lo scambio." },
        { status: 403 },
      )
    }
    ...
    // Trigger: notify the proposer that their trade was accepted.
    await createNotification({
      userId: trade.fromUserId,
      type: "trade",
      title: "Scambio accettato",
      body: `${trade.toUsername || "L'utente"} ha accettato il tuo scambio "${trade.offeredItemName}" ↔ "${trade.requestedItemName}".`,
      link: `/trades/${trade._id}`,
    })

    return NextResponse.json({ success: true, message: "Scambio accettato.", trade })
  } catch (error) {
    console.error("[v0] Errore accept trade:", error)

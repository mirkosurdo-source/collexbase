...
export async function POST(req: Request) {
  try {
    ...
    // Only the buyer or seller of this escrow may trigger release.
    ...
    const result = await releasePayment(escrowId, userId)
    if (!result.ok) {
    ...
    console.error("[v0] wallet/release-payment error:", error)
